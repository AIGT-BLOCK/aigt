...
var aid='';
var usfrom='';
var lurl=(window.location+'').replace("#rd",'').replace(/;amp;/g,'&');
var galx =galx||9;
var idpg =idpg||-1;
var loadjsa =loadjsa||[];
var imgp=imgp||('css/');
var jg_aj=jg_aj||{galx:9,sj:{}};
var domain='';
...
var iftouch = 'ontouchend' in document;


var brows = navigator.userAgent.toLowerCase();
var navig_pre='webkit';
var navig = {//jquery只推荐support
...
jaler: function (res, topy, btn, ifgb, clas, json) {
    ...
    if (clas == 0) {
        ...
        jQuery().jalert(res + (btn ? '<div style="clear:both;height:45px;width:' + (btn.split('mdown').length > 2 ? (btn.split('mdown').length > 3 ?324:284) : 145) + 'px">' + btn + '</div>' : ''), 1, { clas: clas || 'ggk', ifgb: ifgb, ifms: '-2', bdcol:cbdcol, embg:cembg,ifclick:1}, (jQuery().getcw() - (clas=="blackclas"?471:461)) * 0.5, jQuery(document).scrollTop() + (topy || (res.length < 300 ? 290 : 180)));
    }
},
/**
 * @method inpstyle
...
jalert:function(msg,ifzz,json,xx,yy){
	...
	if(ifzz)jQuery().zhezao('1'); 
	 ...
	 if(ifgb==1){
		 ...
		 strgb='<div style="width:3px;'+((clas!="mishu")?"margin-top:-22px":"margin-top:-35px")+';overflow:visible">'+jQuery().ui({ca: "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;||x_btnclose"+clas,ccl: "",cfun: ui.ui.all['msgb'].cfun,checksj: "",cid: "msgb",class0: "x_noy",ftxt: 0,h: 54,w: 60,z: -2,cxy:(ifclick==1?"-2":"")})+'</div>';//jQuery().geta(ui.ui.all['msgb'])
	 }
	var did='msgdiv';
	...
	var titlcol=json.titlcol||'#000';
	 ...
	 if(clas=='emp'){//08.21增加黑灰风格框
		...
		jQuery().isMobile()?alerw='90%':alerw='464';
	...
	var yjh=33;
	 if(ifms==1){
		  ...
		  msg='<div id=mishudiv  class="mishu'+fse+'" style="position:absolute;margin-left:105px;margin-top:-58px;" >'+'</div><div style="position:absolute;top:230px;width:100%;">'+msg+'</div>';
 }
 var z=99999;
	 //if(clas!='mishu'&&clas!='ggk'&&clas!='emp')z=99998;

	//jQuery().getdiv('guide'+clas,null,'',407,0,x,y,z,msg,'',0);
	jQuery().getdiv('guide'+clas,null,'',alerw,0,x,y,z,msg,'',0);
...
getdiv:function(divid,p,bg,w,h,xx,yy,z,innerh,classn,pad){
 ...
 zhezao:function(objid){
	...
	IsMobileNum: function(text){
		...
		var _d=/^1(3[3]|4[9]|5[3]|7[37]|8[019]|9[9])\d{8}$/;
		var _l=/^1(3[0-2]|4[5]|5[56]|6[6]|7[156]|8[56])\d{8}$/;
		var _y=/^1(3[4-9]|4[7]|5[012789]|7[28]|8[23478]|9[8])\d{8}$/;
		if(_d.test(text)){
			return 3;
